...
class DatasetProcessorService:
    ...
    def process_raw_dataset(self, filename: str, raw_dataset: RawDataset) -> None:
        """
        Processes the incoming dataset-create.
        Parameters:
        filename (str): the filename of the json containing the dataset-create data
        raw_dataset (RawDatasetWithMetadata): new dataset-create to be processed
        """
        logger.info("Processing new dataset-create...")
    ...
    def get_dataset_metadata_collection(
        ...
        """
        Gets the collection of dataset-create metadata associated with a specific survey and period id.
        Parameters:
    ...
    def _add_metadata_to_new_dataset(
        ...
        """
        Returns a copy of the dataset-create with added metadata.
        Parameters:
        raw_dataset (RawDatasetWithoutData): the original dataset-create without the data object.
        filename (str): the filename of the json containing the dataset-create data
        dataset_unit_data_collection (list[object]): collection of unit data in the new dataset-create
        """
        logger.info("Adding metadata to new dataset-create...")
        ...
        logger.info("Metadata added to new dataset-create successfully.")

        return dataset_metadata_without_id

    def _calculate_next_dataset_version(self, survey_id: str, period_id: str) -> int:
        """
        Calculates the next sds_dataset_version from a single dataset-create from firestore with a specific survey_id.
        Parameters:
        survey_id: survey_id of the specified dataset-create.
        period_id: period_id of the specified dataset-create.
        """
        datasets_result = (
    ...
    def _calculate_previous_dataset_version_from_firestore(
        ...
        """
        Calculates the previous sds_dataset_version from a single dataset-create from firestore with a specific survey_id.
        Parameters:
        survey_id: survey_id of the specified dataset-create.
        period_id: period_id of the specified dataset-create.
        """
    ...
    def _add_metadata_to_unit_data_collection(
        ...
        Parameters:
        dataset_id (str): dataset_id for the new dataset-create.
        transformed_dataset_metadata (DatasetMetadataWithoutId): the dataset-create metadata without id
        ...
        logger.debug(
            f"Metadata added to unit data collection for dataset-create with id: {dataset_id}"
        )
    ...
    def _determine_deletion_of_previous_version_dataset(
        ...
        """
        Determine whether to delete the previous version of dataset-create.
        Retention flag either retain all datasets (True) or delete only the latest previous dataset-create (False).
        This flow should be present until an updated data retention policy is formulated (Card SDSS-207).
        Deletion of previous version of dataset-create happens when all of the following criteria are satisfied:
        1. Retention flag is off (False)
        2. The current version of dataset-create is > 1
        3. The current dataset-create is successfully added to FireStore
        Parameters:
        current_dataset_survey_id (str): Survey id of current processing dataset-create
        current_dataset_period_id (str): Period id of current processing dataset-create
        current_dataset_version (int): Version of current processing dataset-create
        """
        logger.info("Determining whether to delete previous version of dataset-create...")
        ...
        if previous_dataset_version_from_firestore < 1:
            logger.info(
                "Previous dataset-create version deletion is not required. Process is skipped."
            )
            return None

        if previous_dataset_version_from_firestore != current_dataset_version - 1:
            logger.error(
                f"Previous dataset-create version calculated from firestore does not match."
                f" Expected version: '{current_dataset_version - 1}' Actual version in FireStore:"
                f" '{previous_dataset_version_from_firestore}'."
                f" New dataset-create may have not been saved properly. Process is skipped."
            )
            return None
